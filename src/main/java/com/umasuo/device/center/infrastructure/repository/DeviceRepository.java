...
public interface DeviceRepository extends JpaRepository<Device, String> {
  ...
  @Query("select new map(d.developerId as developerId, d.productId as productId, count(d) as registerCount) from Device d where d.createdAt >= ?1 and d.createdAt < ?2 group by d.developerId, d.productId")
  List<HashMap> getIncreaseReport(long startTime, long endTime);

  @Query("select new map(d.developerId as developerId, d.productId as productId, count(d) as registerCount) from Device d where d.createdAt >= ?2 group by d.developerId, d.productId having d.developerId = ?1")
